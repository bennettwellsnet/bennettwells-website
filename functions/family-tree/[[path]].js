/**
 * Catch-all for /family-tree/* — forces auth before static assets.
 * Password: Cloudflare Pages env FAMILY_TREE_PASSWORD
 */
...
async function serveAsset(context) {
...
export async function onRequest(context) {
  ...
  if (!password) {
    return htmlResponse(
      loginPage({
        error: 'Access is not configured yet (missing FAMILY_TREE_PASSWORD in Cloudflare).',
      }),
      503,
    );
  }
